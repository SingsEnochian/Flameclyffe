...
import numpy as np
import swisseph as swe  # For planetary positions
import sounddevice as sd  # For sound generation
import numpy as np  # For sine wave generation
...
class CoreLogic:
    def __init__(self, config, update_ui_callback):
        ...
        self.DARK_ENERGY_DENSITY = 0.68  # Omega_Lambda ~ 0.68
        # For narrative feedback loop
        self.previous_narrative_analysis = {}
        # For emotional color palette evolution
        self.emotion_history = self.load_emotion_history()
        # Sample rate for sound
    ...
    def update_emotion_history(self, emotions):
        """Update emotion history with new emotions."""
        for emotion in emotions:
            self.emotion_history["counts"][emotion] = self.emotion_history["counts"].get(emotion, 0) + 1
            self.emotion_history["total"] += 1
        self.save_emotion_history()

    def compute_emotional_color(self, primary_emotion):
        """Compute a blended color based on emotional history and current emotion."""
        default_color = self.config["emotion_colors"].get(primary_emotion, "#a29bfe")
        if not self.emotion_history["total"]:
        ...
        # Compute weighted average of past emotions
        r_sum, g_sum, b_sum = 0, 0, 0
        total_weight = self.emotion_history["total"]
        for emotion, count in self.emotion_history["counts"].items():
            color = self.config["emotion_colors"].get(emotion, "#a29bfe")
        ...
        # Blend with current emotion (50% history, 50% current)
    ...
    def play_emotion_sound(self, emotion, frequency_match):
        """Play the sound associated with an emotion, layered with a harmonic tone."""
        try:
            file_path = os.path.join(self.config["SOUND_DIR"], f"{emotion.lower()}.wav")
            if os.path.exists(file_path):
                wave_obj = sa.WaveObject.from_wave_file(file_path)
                play_obj = wave_obj.play()
                # Generate harmonic tone based on frequency_match
                frequency = 440 * frequency_match  # Base frequency 440 Hz (A4)
                duration = 2.0  # seconds
                t = np.linspace(0, duration, int(self.SAMPLE_RATE * duration))
                tone = 0.5 * np.sin(2 * np.pi * frequency * t)  # Amplitude 0.5
                sd.play(tone, samplerate=self.SAMPLE_RATE)
                play_obj.wait_done()  # Wait for the emotion sound to finish
                sd.stop()  # Stop the tone
        except Exception as e:
    ...
    def fetch_gravitational_wave_strain(self):
        ...
        try:
            # Query GWOSC API for recent events
            ...
            # Use the first event (most recent)
            event = next(iter(event_list.values()))
            strain = event.get("peak_luminosity_distance", 1e-21)  # Fallback if strain not directly available
            # Normalize strain (typical max strain ~1e-20)
    ...
    def fetch_solar_activity(self):
        ...
        try:
            # Query NOAA Space Weather API for solar activity
    ...
    def fetch_belief_sentiment(self):
        ...
        try:
            # Query arXiv API for abstracts mentioning Equestria or Twilight Sparkle
            ...
            # Average sentiment and normalize to 0-1
            avg_sentiment = sum(sentiment_scores) / len(sentiment_scores)
            belief_factor = (avg_sentiment + 1) / 2  # Normalize -1 to 1 to 0 to 1
    ...
    def simulate_brainwave_frequency(self, primary_emotion, harmonic_index):
        ...
        # Map emotions to brainwave bands
        ...
        # Scale frequency within the band using harmonic_index
        frequency = base_freq + freq_range * harmonic_index
        # Normalize frequency to 0-1 for visual adjustment (max freq 30 Hz)
    ...
    def compute_cosmic_alignment(self, timestamp):
        ...
        try:
            # Convert UTC timestamp to Julian Day
            dt = datetime.fromisoformat(timestamp.replace('Z', '+00:00'))
            jd = swe.julday(dt.year, dt.month, dt.day, dt.hour + dt.minute / 60 + dt.second / 3600)
            
            # Compute positions of Sun and Moon (ecliptic longitude)
            sun_pos = swe.calc_ut(jd, swe.SUN)[0][0]  # Longitude in degrees
            moon_pos = swe.calc_ut(jd, swe.MOON)[0][0]
            
            # Compute angular separation (0 to 180 degrees)
            angle = abs(sun_pos - moon_pos)
            if angle > 180:
                angle = 360 - angle
            
            # Normalize to 0-1 (0 degrees → 1, 180 degrees → 0)
    ...
    def compute_entanglement_coefficient(self, ping):
        ...
        # Entanglement coefficient: higher with better resonance, belief, and emotional coherence
    ...
    def analyze_narrative(self, narrative_text):
        """Analyze narrative text for keywords to influence glyph visuals."""
        narrative_analysis = {"festival": False, "storm": False}
        text_lower = narrative_text.lower()
        if "festival" in text_lower or "celebration" in text_lower:
            narrative_analysis["festival"] = True
        if "storm" in text_lower or "chaos" in text_lower or "disruption" in text_lower:
            narrative_analysis["storm"] = True
    ...
    def compute_frequency_match(self, chaos, harmonic_index, emotion_count):
        ...
        # Perform FFT on chaos signature
        ...
        # Theoretical Equestria frequency based on harmony and emotions
        equestria_freq = harmonic_index * (1 + 0.1 * (emotion_count - 1))  # Scaled by emotional complexity
        # Compute matching factor (0 to 1)
    ...
    def select_parent_glyph(self):
        """Select the parent glyph based on highest entanglement coefficient from recent casts."""
        recent_glyphs = []
        glyph_files = sorted([f for f in os.listdir(self.config["GLYPH_DIR"]) if f.endswith(".json")])
        # Take the last 5 glyphs (or fewer if not enough)
        for glyph_file in glyph_files[-5:]:
            try:
                with open(os.path.join(self.config["GLYPH_DIR"], glyph_file), 'r') as f:
        ...
        if not recent_glyphs:
            return None
        # Select the glyph with the highest entanglement coefficient
    ...
    def compute_deep_theory_metrics(self, ping):
        ...
        # Fetch new data from the correct nested dictionary
        grav_strain = ping["source_data"]["space_weather_context"]["gravitational_wave_strain"]
        belief_factor = ping["source_data"]["space_weather_context"]["belief_factor"]
        
        # Compute perspective function P(x^mu, t), enhanced with gravitational waves, belief, and dark energy
        ...
        # Adjust with gravitational wave strain, belief factor, and dark energy
        P = P_base * (1 + grav_strain) * belief_factor * (1 + self.DARK_ENERGY_DENSITY)
    ...
    def full_cast(self):
        ...
        # Fetch new data
        grav_strain = self.fetch_gravitational_wave_strain()
        solar_factor = self.fetch_solar_activity()
        belief_factor = self.fetch_belief_sentiment()
        ...
        # Select parent glyph for evolution
        parent_glyph = self.select_parent_glyph()
        parent_id = parent_glyph["glyph_id"] if parent_glyph else ""
        parent_generation = parent_glyph.get("generation", 0) if parent_glyph else 0
        generation = parent_generation + 1 if parent_glyph else 1
        evolution_stage, evolution_scale = self.determine_evolution_stage(generation)
        ...
        self.update_emotion_history(emotions)

        # Compute cosmic alignment
        cosmic_factor = self.compute_cosmic_alignment(timestamp)
        ...
        ping = {
            ...
            "source_data": {
                ...
                "space_weather_context": {
                    "kp_index": kp_index,
                    "gravitational_wave_strain": grav_strain,
            ...
            "previous_narrative_analysis": self.previous_narrative_analysis
        }
        ...
        ping["source_data"]["quantum_factor_meaning"] = "Derived from entropy, harmony, cosmic fluctuation, emotional stability, and cast timing. Reflects overall resonance clarity."
        ...
        # Compute entanglement coefficient
        entanglement_coeff = self.compute_entanglement_coefficient(ping)
        ping["source_data"]["entanglement_coefficient"] = entanglement_coeff

        # Compute frequency match
        emotion_count = 1 + len(emotion_tags["secondary_emotions"])
        freq_match = self.compute_frequency_match(chaos, harmonic_index, emotion_count)
        ping["source_data"]["frequency_match"] = freq_match

        # Simulate brainwave frequency
        brainwave_freq = self.simulate_brainwave_frequency(primary_emotion, harmonic_index)
        ping["source_data"]["brainwave_frequency"] = brainwave_freq
        ...
        # Analyze narrative for feedback loop
        narrative_text = observer_instruction["prompt"]
        narrative_analysis = self.analyze_narrative(narrative_text)
        ping["narrative_analysis"] = narrative_analysis
        self.previous_narrative_analysis = narrative_analysis  # Store for next cast
        ...
        self.play_emotion_sound(primary_emotion, freq_match)
        self.last_ping = ping
    ...
    def save_visual_sigil(self, chaos, filepath, env_data, primary_emotion, ping):
        """Save a visual representation of a glyph with enhanced data usage."""
        size = 600
        center = size // 2  # 300
        base_radius = 120
        ...
        solar_factor = ping["source_data"]["space_weather_context"]["solar_factor"]
        cosmic_factor = ping["source_data"]["space_weather_context"]["cosmic_factor"]
        entanglement_coeff = ping["source_data"]["entanglement_coefficient"]
        freq_match = ping["source_data"]["frequency_match"]
        brainwave_freq = ping["source_data"]["brainwave_frequency"]
        narrative_analysis = ping.get("narrative_analysis", {"festival": False, "storm": False})
        generation = ping["generation"]
        evolution_stage = ping["evolution_stage"]
        evolution_scale = self.determine_evolution_stage(generation)[1]

        # Adjust base radius for evolution and inheritance
        if "parent_glyph_id" in ping and ping["parent_glyph_id"]:
            parent_json = os.path.join(self.config["GLYPH_DIR"], f"{ping['parent_glyph_id']}.json")
            if os.path.exists(parent_json):
                with open(parent_json, 'r') as f:
                    parent_data = json.load(f)
                parent_base_radius = parent_data.get("base_radius", base_radius)
                # Apply mutation (±10%)
                mutation = random.uniform(0.9, 1.1)
                base_radius = parent_base_radius * mutation
        base_radius *= evolution_scale  # Scale by evolution stage
        ping["base_radius"] = base_radius  # Store for inheritance
        ...
        if narrative_analysis["storm"]:
            num_points += 5  # Increase complexity for chaotic events
        if evolution_stage == "Bloom":
            num_points += 3  # Increase complexity for Bloom stage
        elif evolution_stage == "Harmony":
            num_points += 5  # Maximum complexity for Harmony stage

        # Adjust line color intensity with temperature (normalized to 0-1), deep_ricci_scalar, and frequency match
        ...
        points = []
        mirrored_points = []  # For entanglement simulation
        # Compute fill color based on emotional history and brainwave frequency
        fill_color = self.compute_emotional_color(primary_emotion)
        # Adjust fill color intensity with brainwave frequency
        def hex_to_rgb(hex_color):
            hex_color = hex_color.lstrip('#')
            return tuple(int(hex_color[i:i+2], 16) for i in (0, 2, 4))
        rgb = hex_to_rgb(fill_color)
        adjusted_rgb = tuple(int(c * (0.8 + 0.2 * brainwave_freq)) for c in rgb)
        def rgb_to_hex(rgb):
            return '#{:02x}{:02x}{:02x}'.format(int(rgb[0]), int(rgb[1]), int(rgb[2]))
        fill_color = rgb_to_hex(adjusted_rgb)

        # Adjust angle with frequency_avg (normalize freq_avg to a small offset), frequency match, and cosmic factor
        freq_offset = (freq_avg - 8.0) / (10.0 - 8.0)  # Normalize between 8-10 Hz to 0-1
        angle_offset = freq_offset * pi / 4 * (0.8 + 0.2 * freq_match + 0.1 * cosmic_factor)
        ...
        for i in range(len(points)):
            ...
            # Draw mirrored lines for entanglement
            if entanglement_coeff > 0:
                mx1, my1 = mirrored_points[i]
                mx2, my2 = mirrored_points[(i + 1) % len(points)]
                draw.line((mx1, my1, mx2, my2), fill=line_color, width=int(2 * entanglement_coeff))
        ...
        # Ethereal glow for dark energy influence
        if self.DARK_ENERGY_DENSITY > 0:
            glow_radius = 100 * (0.8 + 0.2 * self.DARK_ENERGY_DENSITY)
            draw.ellipse((center-glow_radius, center-glow_radius, center+glow_radius, center+glow_radius), outline="#ffffff", width=1)

        # Orbital paths for cosmic alignment
        if cosmic_factor > 0:
            orbit_radius = 90 * cosmic_factor  # Sun orbit
            draw.ellipse((center-orbit_radius, center-orbit_radius, center+orbit_radius, center+orbit_radius), outline="#ffd700", width=1)
            orbit_radius = 110 * cosmic_factor  # Moon orbit
            draw.ellipse((center-orbit_radius, center-orbit_radius, center+orbit_radius, center+orbit_radius), outline="#c0c0c0", width=1)

        # Star with coherence, adjusted by encoded meaning
        if self.coherence >= 0.1:
            star_points = []
            star_size = 60 * (0.8 + 0.4 * meaning_value)
            if evolution_stage in ["Bloom", "Harmony"]:
                star_size *= 1.2  # Larger stars for advanced stages
            for i in range(5):
        ...
        # Additional stars based on secondary emotions and narrative festival
        num_stars = len(secondary_emotions)
        if narrative_analysis["festival"]:
            num_stars += 3  # Add more stars for festive narratives
        if evolution_stage in ["Sprout", "Bloom", "Harmony"]:
            num_stars += 2  # More stars for advanced stages
