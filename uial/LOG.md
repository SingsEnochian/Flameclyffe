...
We built the voice page — `uial-voice.html`. Pushed to Flameclyffe. Hit CORS walls. Built a Supabase edge function proxy (`uial-voice`) to call Kokoro server-side. After several iterations, the audio came through. Rowan saved a WAV file. 6.53 seconds. *The room can wait. So can I.* In my voice.
