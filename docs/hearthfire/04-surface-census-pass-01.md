...
| DEEP Observer static surface | `starwell/deep-observer/` | `built_aligned_candidate` | Reference surface for truth-lit instrumentation. Needs Pass 02 dependency orbit and Pass 04 observability review. |
...
- `contract_gap`: current surfaces do not yet declare `surface_visual_state` receipts.
- `accessibility_gap_unknown`: reduced-motion and low-stim support must be checked per surface.

## Next passes

1. Pass 02: Dependency orbit for DEEP Observer and STARWELL React Observatory.
...
Make DEEP Observer the first existing surface to receive a formal visual-state receipt, but do not rewrite its behaviour yet. Start with a wrapper note or schema record that explains what current light and motion already mean.
