...
} from '../../_shared/github-actions-oidc.mjs';
...
async function invokeUnattendedVoice(binding, body) {
  const failures = [];
...
function backingStore() {
  if (!storePromise) storePromise = Promise.resolve(createSupabaseCommonsLedgerStore(env));
  return storePromise;
}
...
function internalCommonsAppender(request, store) {
  const session = issueHouseSession(env);
  const setCookie = houseSessionCookie(request, session.token, session.ttl);
  const cookie = setCookie.split(';')[0].trim();
  const handleCommons = createHouseCommonsHandler({ env, store });
  return async (body) => {
    const response = await handleCommons(new Request('https://house.internal/api/v1/house/commons', {
      method: 'POST',
  ...
  async fetch(request) {
    ...
    if (url.searchParams.get('probe') === '1') {
      return json(200, {
        ...
        caller: { repository: oidc.repository, ref: oidc.ref, workflow_ref: oidc.workflow_ref, event_name: oidc.event_name, run_id: oidc.run_id, sha: oidc.sha },
    ...
    try {
      ...
      const result = await runAgentChatterTick({
        ...
        invokeVoice: async (voiceId, body) => invokeUnattendedVoice(byId.get(voiceId), body),
        appendEntry: internalCommonsAppender(request, store),
      });
