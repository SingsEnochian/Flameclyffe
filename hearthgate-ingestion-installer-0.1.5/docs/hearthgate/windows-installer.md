...
- populates rooms, bridges, signals, and the Hearthfire provenance catalogue at first light without overwriting existing per-user data;
- bundles PDF, DOCX, text/document ingestion and offline English image OCR;
- reports API readiness without exposing stored credential values.
...
## Glyph tooling boundary

The installer includes the current Glyph Studio and local FontForge bridge foundation. The glyph-to-font and art system is not complete and must not be described as a finished font compiler or production art pipeline.
