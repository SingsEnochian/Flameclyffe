 ...
 */
(function(global){
  const VERSION='1.8.0',MATH_SPINE='hearthgate.math-spine/v1.8';
  ...
  function orchestrate({
    ...
    const fold=clamp(h.fold_phi??h.phi??0),obs=clamp(h.observation_coherence??0),us=clamp(h.relational_participation??h.us??0),real=clamp(h.realisation??0);

    return Object.freeze({
      schema:'wardenclyffe.layer-orchestration/v1.8',
      version:VERSION,
      math_spine:MATH_SPINE,
      renderer_target:'flameclyffe-mobius',
      state_address:stateAddress,
      stratum,
      lineage,
      temporal,
      crossing:Object.freeze({fold_phi:fold,observation_coherence:obs,relational_participation:us,realisation:real}),
  ...
  function toMobiusSpec(plan){
    ...
    return Object.freeze({
      id:`wardenclyffe-${Date.now()}`,
      label:'Wardenclyffe v1.8 · addressed harmonic braid',
      schema:'mobius.layered-spec/wardenclyffe-v1.8',
      math_spine:MATH_SPINE,
      state_address:plan.state_address,
      stratum:plan.stratum,
      lineage:plan.lineage,
      layers:plan.layers.map(l=>({...l})),
      wardenclyffe:{temporal:plan.temporal,crossing:plan.crossing,haptics:plan.haptics,provenance:plan.provenance}
    });
  }
