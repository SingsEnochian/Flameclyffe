...
/* Heimdall Sonification Compiler v0.2 · Hearthgate Math v1.8
 * Reads local fold geometry + observation coupling + state/gate address + stratigraphy.
 * Runa composes; Wardenclyffe orchestrates; Möbius renders; Heimdall supplies turning geometry.
 */
(function(global){
  const VERSION='0.2.0';
  ...
  function validateBlocks(blocks,n){
    ...
    for(const b of blocks)if(!b?.id||!Number.isInteger(b.start)||!Number.isInteger(b.end)||b.start<0||b.end<=b.start||b.end>n)throw new RangeError(`Invalid Heimdall block ${b?.id||'?'}`);
  }
  function blockParticipation(vMin,blocks){
    validateBlocks(blocks,vMin.length); const total=Math.max(EPS,norm2(vMin));
  ...
  function couplingTopology(matrix,blocks){
    ...
    validateBlocks(blocks,matrix.length); const out={};
    for(const dst of blocks){out[dst.id]={};for(const src of blocks){if(src.id===dst.id)continue;const sub=[];for(let r=dst.start;r<dst.end;r++)sub.push((matrix[r]||[]).slice(src.start,src.end));out[dst.id][src.id]=frob(sub);}}
    return Object.freeze(out);
  }
  function observationCoherence(witnesses=[]){
    const math=global.HearthgateMathV18; if(math?.observationCoherence)return math.observationCoherence(witnesses);
    const rows=witnesses.filter(Array.isArray).filter(v=>v.length); if(rows.length<2)return rows.length?1:0;
    const n=Math.min(...rows.map(r=>r.length)); const mean=Array.from({length:n},(_,j)=>rows.reduce((s,r)=>s+(Number(r[j])||0),0)/rows.length);
    let spread=0,total=0; for(const r of rows)for(let j=0;j<n;j++){const x=Number(r[j])||0;spread+=(x-mean[j])**2;total+=x*x;} return clamp(1-spread/(total+EPS));
  }
  ...
  function compileHeimdallSonification(input={},opts={}){
    ...
    if(!vMin.length||!singularValues.length)throw new TypeError('Heimdall singularValues and vMin are required.');
