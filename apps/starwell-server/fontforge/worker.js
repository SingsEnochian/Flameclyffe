...
function glyphBounds(glyph) {
  ...
  for (const stroke of strokes) {
    stroke.points.forEach((point, index) => {
      const radius = pointWidth(stroke, point, index) / 2;
      minX = Math.min(minX, Number(point.x) - radius);
      minY = Math.min(minY, Number(point.y) - radius);
      maxX = Math.max(maxX, Number(point.x) + radius);
      maxY = Math.max(maxY, Number(point.y) + radius);
    });
  }
...
function validateFontProject(project) {
  const errors = [];
  const warnings = [];
  const glyphReports = [];
  const glyphs = Array.isArray(project?.glyphs) ? project.glyphs : [];
  if (!glyphs.length) errors.push({ code: 'project-empty', message: 'At least one glyph is required.' });
  if (glyphs.length > MAX_GLYPHS) errors.push({ code: 'project-too-large', message: `Projects are limited to ${MAX_GLYPHS} glyphs.` });
  const seen = new Map();
  glyphs.forEach((glyph, index) => {
    ...
    let codepoint = null;
    try {
      codepoint = parseCodepoint(glyph.codepoint || (0xe000 + index).toString(16));
      if (seen.has(codepoint.number)) glyphErrors.push(`Duplicate codepoint U+${codepoint.hex}; also used by ${seen.get(codepoint.number)}.`);
      else seen.set(codepoint.number, glyph.name || `glyph ${index + 1}`);
      const expectedCharacter = String.fromCodePoint(codepoint.number);
      if (glyph.character && glyph.character !== expectedCharacter) glyphWarnings.push(`Display character does not match U+${codepoint.hex}.`);
    } catch (error) { glyphErrors.push(error.message); }
    const layers = Array.isArray(glyph.layers) ? glyph.layers : [];
    const eligible = layers.filter(eligibleLayer);
    const excluded = layers.filter((layer) => !eligibleLayer(layer));
    const bounds = glyphBounds(glyph);
    const advanceWidth = Number(glyph.advanceWidth ?? EM_SIZE);
    if (!Number.isFinite(advanceWidth) || advanceWidth < 0 || advanceWidth > 4000) glyphErrors.push('Advance width must be between 0 and 4000 units.');
    if (!eligible.length) glyphWarnings.push('No eligible vector layer; this glyph will compile blank.');
    else if (!bounds) glyphWarnings.push('Eligible vector layers contain no drawable points; this glyph will compile blank.');
    if (excluded.length) glyphWarnings.push(`${excluded.length} layer${excluded.length === 1 ? '' : 's'} will be excluded from compilation.`);
    if (bounds) {
      if (bounds.minX < 0 || bounds.maxX > advanceWidth) glyphWarnings.push(`Outline crosses its advance box (${bounds.minX.toFixed(1)}–${bounds.maxX.toFixed(1)} of ${advanceWidth}).`);
      if (bounds.minY < 0 || bounds.maxY > EM_SIZE) glyphWarnings.push(`Outline crosses the 1000-unit vertical em box (${bounds.minY.toFixed(1)}–${bounds.maxY.toFixed(1)}).`);
      const measuredLeft = Math.round(bounds.minX);
      const measuredRight = Math.round(advanceWidth - bounds.maxX);
      if (Math.abs(Number(glyph.leftBearing ?? measuredLeft) - measuredLeft) > 10) glyphWarnings.push(`Recorded left bearing ${glyph.leftBearing} differs from drawn bearing ${measuredLeft}.`);
      if (Math.abs(Number(glyph.rightBearing ?? measuredRight) - measuredRight) > 10) glyphWarnings.push(`Recorded right bearing ${glyph.rightBearing} differs from drawn bearing ${measuredRight}.`);
    }
    const label = `${glyph.name || `Glyph ${index + 1}`}${codepoint ? ` (U+${codepoint.hex})` : ''}`;
    glyphErrors.forEach((message) => errors.push({ code: 'glyph-error', glyphId: glyph.id, message: `${label}: ${message}` }));
    glyphWarnings.forEach((message) => warnings.push({ code: 'glyph-warning', glyphId: glyph.id, message: `${label}: ${message}` }));
    glyphReports.push({ glyphId: glyph.id, name: glyph.name || `Glyph ${index + 1}`, codepoint: codepoint?.hex || null, eligibleLayerCount: eligible.length, excludedLayerCount: excluded.length, bounds, errors: glyphErrors, warnings: glyphWarnings });
  });
  const glyphIds = new Set(glyphs.map((glyph) => String(glyph.id || '')));
  const pairKeys = new Set();
  const kerningPairs = Array.isArray(project?.kerningPairs) ? project.kerningPairs : [];
  kerningPairs.forEach((pair, index) => {
    const label = `Kerning pair ${index + 1}`;
    if (!glyphIds.has(String(pair.leftGlyphId || '')) || !glyphIds.has(String(pair.rightGlyphId || ''))) errors.push({ code: 'kerning-reference', message: `${label} references a missing glyph.` });
    const key = `${pair.leftGlyphId}:${pair.rightGlyphId}`;
    if (pairKeys.has(key)) errors.push({ code: 'kerning-duplicate', message: `${label} duplicates an existing left/right pair.` });
    pairKeys.add(key);
    const value = Number(pair.value);
    if (!Number.isFinite(value) || value < -1000 || value > 1000) errors.push({ code: 'kerning-value', message: `${label} must be between -1000 and 1000 units.` });
  });
  const ligatures = Array.isArray(project?.ligatures) ? project.ligatures : [];
  const ligatureKeys = new Set();
  ligatures.forEach((rule, index) => {
    const label = `Ligature ${index + 1}`;
    const inputs = Array.isArray(rule.inputGlyphIds) ? rule.inputGlyphIds.map(String) : [];
    if (inputs.length < 2) errors.push({ code: 'ligature-input', message: `${label} requires at least two input glyphs.` });
    if (inputs.some((id) => !glyphIds.has(id)) || !glyphIds.has(String(rule.outputGlyphId || ''))) errors.push({ code: 'ligature-reference', message: `${label} references a missing glyph.` });
    const feature = ['liga', 'dlig', 'rlig'].includes(rule.feature) ? rule.feature : 'liga';
    const key = `${feature}:${inputs.join('+')}`;
    if (ligatureKeys.has(key)) errors.push({ code: 'ligature-duplicate', message: `${label} duplicates an existing ${feature} sequence.` });
    ligatureKeys.add(key);
  });
  return { schemaVersion: 'starwell.fontforge.preflight.v0.1', ok: errors.length === 0, projectId: String(project?.id || ''), glyphCount: glyphs.length, kerningPairCount: kerningPairs.length, ligatureCount: ligatures.length, errors, warnings, glyphs: glyphReports, checkedAt: new Date().toISOString() };
}
...
function buildFontJob(project, options = {}) {
  ...
  const preflight = validateFontProject(project);
  if (!preflight.ok) throw new Error(`Font preflight failed: ${preflight.errors.map((item) => item.message).join(' · ')}`);
  ...
  const glyphNameById = new Map(glyphs.map((glyph) => [glyph.id, glyph.glyphName]));
  const kerningPairs = (Array.isArray(project.kerningPairs) ? project.kerningPairs : []).map((pair) => ({
    leftGlyphName: glyphNameById.get(String(pair.leftGlyphId)),
    rightGlyphName: glyphNameById.get(String(pair.rightGlyphId)),
    value: clamp(pair.value, -1000, 1000),
  }));
  const ligatures = (Array.isArray(project.ligatures) ? project.ligatures : []).map((rule) => ({
    feature: ['liga', 'dlig', 'rlig'].includes(rule.feature) ? rule.feature : 'liga',
    inputGlyphNames: rule.inputGlyphIds.map((id) => glyphNameById.get(String(id))),
    outputGlyphName: glyphNameById.get(String(rule.outputGlyphId)),
  }));
  ...
  return {
    ...
    ligatures,
    totalPoints,
    preflight: { warningCount: preflight.warnings.length, checkedAt: preflight.checkedAt },
...
receipt = {
    ...
    "errors": [],
    "warnings": [],
    "features": {"kerningPairs": len(job.get("kerningPairs", [])), "ligatures": len(job.get("ligatures", []))},
}
...
try:
    ...
    kerning_pairs = job.get("kerningPairs", [])
    if kerning_pairs:
        font.addLookup("STARWELL-kern", "gpos_pair", (), (("kern", (("latn", ("dflt",)),)),))
        font.addLookupSubtable("STARWELL-kern", "STARWELL-kern-1")
        for pair in kerning_pairs:
            font[pair["leftGlyphName"]].addPosSub("STARWELL-kern-1", pair["rightGlyphName"], int(pair["value"]))

    ligatures_by_feature = {}
    for rule in job.get("ligatures", []):
        ligatures_by_feature.setdefault(rule.get("feature", "liga"), []).append(rule)
    for feature, rules in ligatures_by_feature.items():
        lookup = "STARWELL-" + feature
        subtable = lookup + "-1"
        font.addLookup(lookup, "gsub_ligature", (), ((feature, (("latn", ("dflt",)),)),))
        font.addLookupSubtable(lookup, subtable)
        for rule in rules:
            font[rule["outputGlyphName"]].addPosSub(subtable, tuple(rule["inputGlyphNames"]))
