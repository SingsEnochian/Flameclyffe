...
app.post('/api/fontforge/validate', (req, res) => {
  try {
    const report = validateFontProject(req.body?.project);
    res.status(report.ok ? 200 : 422).json({ ok: report.ok, report });
  } catch (error) {
    res.status(400).json({ ok: false, error: error.message, timestamp: new Date().toISOString() });
  }
});
