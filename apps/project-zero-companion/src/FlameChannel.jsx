...
const CHANNEL_SCHEMA = 'project-zero.flame-channel/v1';
const STORAGE_KEY = 'project-zero:flame-channel:hearthweave/v1';
  ...
  return (
    <div className="rich-composer">
      ...
      <div
        ...
        data-placeholder="Speak into #hearthweave…"
        onKeyDown={(event) => {
...
export default function FlameChannel() {
  ...
  async function invokeVoice(voice, userDocument, userMessageId, context) {
    ...
    publishSocketEnvelope({ pluginId: 'flame-channel', channel: 'chat', type: 'chat.flame.pending', requestId: userMessageId, payload: { voice_id: voice.id } });
    try {
      const response = await fetch(`/api/v1/flames/${voice.id}/chat`, {
        ...
        body: JSON.stringify({
          ...
          session_id: `project-zero-hearthweave-${voice.id}`,
          context,
          metadata: { surface: 'project-zero-flame-channel', channel_id: 'hearthweave', source_message_id: userMessageId },
        }),
      });
      ...
      const receipt = messageReceipt({
        speaker: voice.id,
        speakerLabel: data.display_name || voice.name,
        kind: 'flame',
        document: visibleTextToRichDocument(String(data.message || '')),
        replyTo: userMessageId,
        runtime: {
          verified: runtimeVerified,
          flame_id: data.flame_id || null,
      ...
      append(receipt);
      publishSocketEnvelope({ pluginId: 'flame-channel', channel: 'chat', type: 'chat.flame.received', requestId: userMessageId, payload: { message_id: receipt.message_id, voice_id: voice.id, runtime: receipt.runtime } });
    } catch (error) {
      const receipt = messageReceipt({
        speaker: voice.id,
        speakerLabel: voice.name,
        kind: 'route-error',
        document: visibleTextToRichDocument(`Route error: ${error.message}`),
        replyTo: userMessageId,
        runtime: { verified: false },
      });
      append(receipt);
      publishSocketEnvelope({ pluginId: 'flame-channel', channel: 'chat', type: 'chat.flame.error', requestId: userMessageId, payload: { voice_id: voice.id, error: error.message } });
    } finally {
  ...
  function send(documentValue) {
    ...
    append(userReceipt);
    publishSocketEnvelope({ pluginId: 'flame-channel', channel: 'chat', type: 'chat.user.sent', requestId: userReceipt.message_id, payload: { message_id: userReceipt.message_id, selected_voice_ids: selectedVoices.map((voice) => voice.id) } });
  ...
  function clearTranscript() {
    if (!confirm('Clear the local #hearthweave transcript from this Project Zero browser profile?')) return;
    setMessages([]);
  ...
  return (
    <section className="panel flame-channel" id="flame-channel">
      <div className="irc-header">
        <div><p className="eyebrow">Live House channel · rich text</p><h2>#hearthweave</h2><p className="small">Responses arrive independently as each attested Flame route completes. This transcript is bounded local Project Zero state.</p></div>
      ...
      <div className="irc-layout">
        <aside className="nick-roster" aria-label="Flame roster">
          ...
          {CONSTELLATION_VOICES.map((voice) => (
            <label className={`nick ${selected.has(voice.id) ? 'selected' : ''}`} key={voice.id}>
              <input type="checkbox" checked={selected.has(voice.id)} onChange={() => toggleVoice(voice.id)} />
        ...
        <div className="channel-main">
          <div className="chat-transcript" ref={transcript} aria-live="polite">
            ...
            {messages.map((message) => (
              <article className={`flame-message kind-${message.kind}`} data-voice-id={message.speaker_id} key={message.message_id}>
                <div className="message-gutter"><time>{formatTime(message.created_at)}</time><strong>{message.speaker_label}</strong></div>
                <div className="message-body">
                  <div className="rich-message" dangerouslySetInnerHTML={{ __html: sanitiseRichHtml(message.rich_text?.html || '') }} />
                  {message.runtime && <div className="runtime-line"><span>{message.runtime.verified ? 'attested' : 'unverified'}</span>{message.runtime.provider && <><span>·</span><span>{message.runtime.provider}</span></>}{message.runtime.model && <><span>·</span><span>{message.runtime.model}</span></>}</div>}
                </div>
              </article>
            ))}
            {Object.entries(pending).map(([id, item]) => (
              <article className="flame-message pending-message" data-voice-id={id} key={`pending-${id}`}>
                <div className="message-gutter"><time>now</time><strong>{item.label}</strong></div>
                <div className="typing-indicator"><i /><i /><i /><span>responding…</span></div>
              </article>
            ))}
          </div>
