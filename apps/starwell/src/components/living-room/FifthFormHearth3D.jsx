...
function TetraShell({ radius, color, glow, opacity, invert = false }) {
  ...
  return (
    <group rotation={rotation}>
      ...
      {edges.map((points, index) => (
        <Line key={index} points={points} color={glow} lineWidth={1.15} transparent opacity={0.68} />
      ))}
    </group>
  );
}
...
function MerkabaLayer({ layer, lowMotion }) {
  ...
  return (
    <group ref={ref} position={layer.position} scale={layer.scale} rotation={layer.rotation}>
      <TetraShell radius={layer.radius} color={layer.color} glow={layer.glow} opacity={layer.opacity} />
      <TetraShell radius={layer.radius * 0.98} color={layer.key === 'core' ? '#1a4d3a' : '#070c09'} glow={layer.key === 'core' ? '#78eec4' : '#d49b54'} opacity={layer.opacity * 0.9} invert />
    </group>
  );
}
  ...
  return (
    <group ref={ref}>
      {threads.map((points, index) => (
        <Line key={index} points={points} color={index % 2 ? '#c98755' : '#2d7a5f'} lineWidth={0.95} transparent opacity={0.28} />
      ))}
    </group>
  );
}

function FieldRings({ lowMotion }) {
  const ref = useRef();
  ...
  return (
    <group ref={ref}>
      <mesh rotation={[0.54, 0, 0]}>
        ...
        <meshBasicMaterial color="#78eec4" transparent opacity={0.12} />
      </mesh>
      <mesh rotation={[1.22, 0.34, 0.74]}>
        <torusGeometry args={[3.12, 0.01, 12, 96]} />
        <meshBasicMaterial color="#c98755" transparent opacity={0.10} />
      </mesh>
      <mesh rotation={[0.18, 1.22, 1.42]}>
        <torusGeometry args={[2.72, 0.01, 12, 96]} />
        <meshBasicMaterial color="#e7c477" transparent opacity={0.08} />
      </mesh>
    </group>
  );
}
...
function FifthFormScene({ lowMotion = false }) {
  ...
  return (
    <>
      ...
      <pointLight position={[-2.8, 1.8, 2.8]} intensity={2.6} color="#2d7a5f" />
      <pointLight position={[2.4, -1.7, 2.2]} intensity={1.8} color="#c98755" />
      <spotLight castShadow position={[0, 4.3, 3.2]} angle={0.42} penumbra={0.72} intensity={1.65} color="#e7c477" />

      <group ref={fieldRef} rotation={[-0.18, 0, 0]}>
        <FieldRings lowMotion={lowMotion} />
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -2.05, 0]} receiveShadow>
        ...
        <meshStandardMaterial color="#100c08" emissive="#2d1a0e" emissiveIntensity={0.22} roughness={0.82} metalness={0.35} transparent opacity={0.86} />
      </mesh>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -2.01, 0]}>
        <ringGeometry args={[1.45, 2.85, 96]} />
        <meshBasicMaterial color="#c98755" transparent opacity={0.14} side={THREE.DoubleSide} />
      </mesh>
    </>
  );
}
