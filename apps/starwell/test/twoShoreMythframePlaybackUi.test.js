...
test('Mythframe playback preserves embodied Shokz routing and user-controlled looping', async () => {
  ...
  assert.equal(manifest.instrumentContract.shokzPlaybackRequiresUserConfirmation, true);
  assert.equal(manifest.relationContract.elevenYearLoopUserControlled, true);
...
test('the live tone event drives a coil compression, release, and upward spiral animation', async () => {
  ...
  assert.equal(manifest.relationContract.coilReleaseVisualDerivedFromToneEvent, true);
