...
test('accepted self-authored cells remain reviewed runtime cells, not stable core', () => {
  const proposal = {
    ...
    receipt: {
      route: 'uial', provider: 'test', model: 'test-model', responseText: '{}', citedSources: [],
    },
    claims: [{
      cellType: 'preference',
      predicate: 'prefers',
...
test('self-authorship prompt forbids authoring other people or canon and hidden reasoning', () => {
  ...
  assert.match(prompt, /Do not author facts for another person/);
  assert.match(prompt, /Do not mark anything stable_core/);
