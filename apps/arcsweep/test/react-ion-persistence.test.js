...
test('default and normalised Arcsweep state carry versioned React-ion persistence slots', () => {
...
test('saveState merges current registry and Helm sidecar stores into the primary snapshot', async () => {
...
test('importing Arcsweep state seeds the sidecar stores from the imported React-ion ledger', async () => {
  const storage = withStorage();
  try {
    const state = createDefaultState();
