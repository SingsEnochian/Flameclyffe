...
const DEFAULT_CONFIG = fileURLToPath(new URL('../skills/sources/hollow-vale/site-ingest.json', import.meta.url));
...
function meta(html, name) {
  ...
  const p1 = new RegExp('<meta[^>]+(?:name|property)=[\"\']' + escaped + '[\"\'][^>]+content=[\"\']([^\"\']*)[\"\'][^>]*>', 'i');
  const p2 = new RegExp('<meta[^>]+content=[\"\']([^\"\']*)[\"\'][^>]+(?:name|property)=[\"\']' + escaped + '[\"\'][^>]*>', 'i');
...
export function isHollowValeRelevant(urlLike, title = '', text = '', config) {
  ...
  return config.crawl.relevance_terms.some((term) => haystack.includes(term.toLowerCase()));
}
...
export function classifySource(urlLike, config) {
  ...
  return 'hollow-vale-related-reference';
}

function semanticSignals(title = '', text = '') {
  const haystack = (title + '\n' + text).toLowerCase();
  const table = {
    character: ['caelwyn', 'father thorn', 'dunval', 'caradoc'],
    language: ['tharionese', 'wyrdsong', 'forgotten tongue', 'rune'],
    memory: ['memory', 'remember', 'forgetting', 'bell of memory'],
    identity: ['identity', 'name', 'names', 'belonging'],
    artefact: ['rune-stone', 'starforged bell', 'bell of memory'],
    cosmology: ['wyrd', 'deep wyrd', 'elysian convergence', 'breaking of the stars'],
    group: ['daughters of avalon'],
    place: ['elderglen', 'tharion', 'hollow vale', 'avalon'],
    myth_variant: ['legend', 'alternative origin', 'some claim', 'another tale']
  };
...
async function main() {
  ...
  if (args.dryRun) {
    console.log(JSON.stringify({ corpus_id: config.corpus_id, sitemap_urls: sitemapUrls.length, seed_count: seeds.size }, null, 2));
    return;
  }
  ...
  while (queue.length && visited.size < maxPages) {
    ...
    try {
      ...
      if (!isHollowValeRelevant(canonical, title, text, config)) continue;

      const sourceHash = hash(bytes);
      const sourceId = 'hollow-vale:' + sourceHash.slice(0, 20);
      relevant.push({
        source_id: sourceId,
        url: finalUrl,
        ...
        semantic_signals: semanticSignals(title, text),
