...
function ensureLedger(state) {
  state.glyphContinuity ||= {
    schema: 'glyph.continuity-ledger/v1',
    version: 1,
    heartbeats: [],
    blindPairs: [],
  };
  state.glyphContinuity.heartbeats ||= [];
  state.glyphContinuity.blindPairs ||= [];
  return state.glyphContinuity;
}
...
function latestWorldEntry(ledger, worldId) {
  return [...ledger.heartbeats].reverse().find((entry) => entry?.world_id === worldId) || null;
}
...
function glyphSvg(signature) {
  if (!signature?.render?.points?.length) return '';
...
function comparisonSummary(pair) {
  ...
  return `<div class="glyph-blind-result">
    ...
    <dl class="facts"><div><dt>Lexical overlap</dt><dd>${Number(comparison.metrics.lexical_jaccard).toFixed(3)}</dd></div><div><dt>Length ratio</dt><dd>${Number(comparison.metrics.length_ratio).toFixed(3)}</dd></div><div><dt>Earth tokens</dt><dd>${comparison.metrics.earth_token_count}</dd></div><div><dt>Return tokens</dt><dd>${comparison.metrics.return_token_count}</dd></div></dl>
    <div class="grid two compact-grid"><article><p class="eyebrow">Earth narrative · revealed</p><p>${esc(pair.earth.text)}</p></article><article><p class="eyebrow">Return narrative · revealed</p><p>${esc(pair.return.text)}</p></article></div>
...
function blindPanel(ledger, worldId) {
  ...
  const latestCompleted = [...ledger.blindPairs].reverse().find((item) => item?.world_id === worldId && item?.comparison) || null;

  if (!pair) {
    return `<div class="glyph-blind-stage">
      <p class="muted">Seal the first narrative before the return side is written. After sealing, the first prose leaves the visible return-stage context and only its hash remains.</p>
      ...
      ${latestCompleted ? comparisonSummary(latestCompleted) : ''}
    </div>`;
  }
  ...
  if (!pair.return?.seal) {
    return `<div class="glyph-blind-stage">
      ...
      <p class="muted">The return stage receives the Earth seal and hash. Earth prose stays withheld until the return narrative is sealed.</p>
      <form data-glyph-return-seal class="stack compact-stack">
        <label>Return narrative<textarea name="returnText" rows="5" required placeholder="Record the independent return before reveal."></textarea></label>
  ...
  return `<div class="glyph-blind-stage">
    <p class="callout"><b>Both sides sealed.</b></p>
    <dl class="facts"><div><dt>Earth hash</dt><dd><code>${esc(pair.earth.seal.content_hash)}</code></dd></div><div><dt>Return hash</dt><dd><code>${esc(pair.return.seal.content_hash)}</code></dd></div></dl>
    <p class="muted">The reveal gate is open. Comparison reads the two sealed texts, verifies both hashes, and writes a comparison receipt.</p>
...
async function renderPanel(message = '') {
  ...
  return `<section class="panel glyph-drift-observatory" data-glyph-drift-observatory>
    <div class="section-heading compact-heading"><div><p class="eyebrow">Observer heartbeat · Living Glyph</p><h2>Glyph Drift Observatory</h2><p class="muted">PREMAQC becomes a deterministic glyph signature. Heartbeats establish a continuity envelope. Structural movement, semantic movement, topology change, and directional trend are receipted separately.</p></div></div>
    ${message ? `<p class="callout">${esc(message)}</p>` : ''}
    <div class="grid two glyph-observatory-grid">
      <article class="glyph-stage">
        ${signature ? glyphSvg(signature) : '<div class="glyph-empty">No glyph heartbeat yet.</div>'}
        ...
        ${driftFacts(drift)}
        <button type="button" data-glyph-heartbeat ${premaqc ? '' : 'disabled'}>Receipt glyph heartbeat</button>
      ...
      <article>
        <p class="eyebrow">Continuity wake</p>
...
document.addEventListener('click', async (event) => {
  ...
  if (heartbeatButton) {
    const panel = heartbeatButton.closest('[data-glyph-drift-observatory]');
    ...
    try {
      ...
      const drift = await classifyGlyphDrift({
        history: priorEntries.map((entry) => entry.heartbeat),
        current: heartbeat,
      });
      ledger.heartbeats.push({
        schema: 'glyph.continuity-entry/v1',
        world_id: world.id,
        heartbeat,
        drift,
      });
      if (ledger.heartbeats.length > MAX_HEARTBEATS) ledger.heartbeats.splice(0, ledger.heartbeats.length - MAX_HEARTBEATS);
      await saveState(state, { reason: 'glyph-continuity-heartbeat' });
      await rerender(panel, `${drift.classification} · ${heartbeat.signature.signature_id}`);
    } catch (error) {
      await rerender(panel, `Glyph heartbeat stopped: ${error.message}`);
    }
    return;
  }
  ...
  if (revealButton) {
    const panel = revealButton.closest('[data-glyph-drift-observatory]');
    ...
    try {
      ...
      await saveState(state, { reason: 'glyph-blind-comparison-reveal' });
      await rerender(panel, `Blind comparison receipted: ${pair.comparison.comparison_id}`);
    } catch (error) {
      await rerender(panel, `Blind comparison stopped: ${error.message}`);
    }
  }
});
...
document.addEventListener('submit', async (event) => {
  ...
  if (earthForm) {
    ...
    const panel = earthForm.closest('[data-glyph-drift-observatory]');
    try {
      const state = await loadState();
      const ledger = ensureLedger(state);
      const { world } = activeContext(state);
      ...
      ledger.blindPairs.push({
        schema: 'glyph.blind-pair/v1',
        pair_id: `glyph-blind-pair-${seal.fingerprint.slice(0, 24)}`,
        world_id: world.id,
        ...
        comparison: null,
        context_gate: {
          return_side_receives: ['earth.seal_id', 'earth.content_hash'],
          earth_prose_withheld_until_return_sealed: true,
        },
      });
      await saveState(state, { reason: 'glyph-blind-earth-seal' });
      await rerender(panel, `Earth narrative sealed: ${seal.seal_id}`);
    } catch (error) {
      await rerender(panel, `Earth seal stopped: ${error.message}`);
    }
    return;
  }
  ...
  if (returnForm) {
    event.preventDefault();
    const panel = returnForm.closest('[data-glyph-drift-observatory]');
    try {
      const state = await loadState();
      const ledger = ensureLedger(state);
      const { world } = activeContext(state);
      ...
      await saveState(state, { reason: 'glyph-blind-return-seal' });
      await rerender(panel, `Return narrative sealed: ${seal.seal_id}. Reveal gate open.`);
    } catch (error) {
      await rerender(panel, `Return seal stopped: ${error.message}`);
    }
  }
});
