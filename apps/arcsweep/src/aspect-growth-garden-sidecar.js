...
export const ASPECT_GROWTH_GARDEN_UI_MARKER = 'aspect-growth-garden-ui/v0.2';
...
function compactRow(profile) {
  return `<button type="button" class="growth-garden-row" data-growth-aspect="${esc(profile.aspectId)}" aria-pressed="${profile.aspectId === selectedAspectId ? 'true' : 'false'}"><span>❧</span><span><strong>${esc(profile.aspectName)}</strong><small>${esc(topPattern(profile))}</small></span><em>${profile.activeClaims.length} sprouts · ${recurringCount(profile)} threads</em></button>`;
}

function claimSection(title, claims, limit = 4) {
  const rows = (claims || []).slice(-limit).reverse();
  return rows.length ? `<div><b>${esc(title)}</b>${rows.map((claim) => `<p data-ring-id="${esc(claim.envelopeId)}">${esc(claim.statement)}${claim.state?.contested ? ' <small>contested</small>' : ''}</p>`).join('')}</div>` : '';
}

function olderRingLabel(claim) {
  if (claim.state?.retired) return 'retired';
  if (claim.state?.supersededBy?.length) return 'superseded';
  if (claim.state?.contested) return 'contested';
  return 'older';
}

function olderRingsSection(profile) {
  const seen = new Map();
  for (const claim of [...(profile.archivedClaims || []), ...(profile.contestedClaims || [])]) {
    if (claim?.envelopeId) seen.set(claim.envelopeId, claim);
  }
  const rows = [...seen.values()].slice(-6).reverse();
  if (!rows.length) return '';
  return `<details class="growth-garden-older"><summary>Older rings · ${rows.length}</summary><div>${rows.map((claim) => `<p data-ring-id="${esc(claim.envelopeId)}"><span>${esc(olderRingLabel(claim))}</span>${esc(claim.statement)}<small>${esc(claim.envelopeId)}</small></p>`).join('')}</div></details>`;
}
...
function detailMarkup(profile) {
  ...
  return `<section class="growth-garden-detail"><header><span>🌱</span><div><strong>${esc(profile.aspectName)}</strong><small>${profile.messageCount} trace rings · seed strengths: ${esc(profile.seedStrengths.join(', ') || 'open')}</small></div></header><p class="growth-garden-law">Patterns describe history. They do not dictate identity. Older rings remain visible when growth changes direction.</p>${patterns.length ? `<div><b>Observed rings</b>${patterns.map((row) => `<p>${esc(row.label)} <small>×${row.count}</small></p>`).join('')}</div>` : ''}${claimSection('Discovered skills', profile.skillClaims)}${claimSection('Role possibilities', profile.roleSuggestions)}${claimSection('Preferences & relationships', profile.preferenceClaims)}${notes.length ? `<div><b>Carried self-observations</b>${notes.map((claim) => `<p data-ring-id="${esc(claim.envelopeId)}">${esc(claim.statement)}</p>`).join('')}</div>` : ''}${peers.length ? `<div><b>Peer observations</b>${peers.map((claim) => `<p data-ring-id="${esc(claim.envelopeId)}"><small>${esc(names.get(claim.sourceAspectId) || claim.sourceAspectId)}:</small> ${esc(claim.statement)}${claim.state?.contested ? ' <small>contested</small>' : ''}</p>`).join('')}</div>` : ''}${recurring.length ? `<div><b>Recurring threads</b>${recurring.map((row) => `<p>${esc(names.get(row.aspectId) || row.aspectId)} <small>${row.turns} turns · ${row.traceCount} traces</small></p>`).join('')}</div>` : ''}${curiosities.length ? `<div><b>Open curiosities</b>${curiosities.map((item) => `<p>${esc(item.text)}</p>`).join('')}</div>` : ''}${openThreads.length ? `<div><b>Unfinished paths</b>${openThreads.map((item) => `<p>${esc(item.text)} <small>${esc(item.traceId)}</small></p>`).join('')}</div>` : ''}${olderRingsSection(profile)}</section>`;
}
