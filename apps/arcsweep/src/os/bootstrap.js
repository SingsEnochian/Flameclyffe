...
function installArcSweepOS() {
  ...
  async function inspect() {
    ...
    if (findings.length) dispatchDomEvent('arcsweep:caretaker-findings', { findings });
    return findings;
  }
  ...
  const api = Object.freeze({
    ...
    session: () => session,
    capsules: () => capsules.map((item) => structuredClone(item)),
    lastNavigationReceipt: () => lastNavigationReceipt ? structuredClone(lastNavigationReceipt) : null,
    navigate,
    inspect,
    setFeatherPaused,
  });
