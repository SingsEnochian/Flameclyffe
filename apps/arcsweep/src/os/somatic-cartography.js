...
export const SOMATIC_CARTOGRAPHY_RECEIPT_SCHEMA = 'arcsweep.somatic-cartography-receipt/v1';
...
function clean(value, fallback = null) { const text = String(value ?? '').trim(); return text || fallback; }
function cleanList(values = []) { return [...new Set((values || []).map((item) => clean(item)).filter(Boolean))]; }
...
export const KELYRAN_SOMATIC_PROFILE = createSomaticProfile({
  ...
  rhythm: { base_bpm: 55, tracing_meter: '2+3' },
  constraints: { seated_supported: true, large_neck_motion_required: false },
});

function desiredGesture(target) { return clean(target?.desired?.gesture_id || target?.desired?.gesture); }
function arrivalConditions(target) { return cleanList(target?.arrival_conditions || []); }
...
export function calculateSomaticCourse({ state, target, profile }, { now = () => new Date() } = {}) {
  ...
  if (gesture) {
    ...
    push('tracing-ready → embodied-glyph', ['glyphforge.trace.arm', 'runa.audio.play', 'runa.haptic.pattern'], {
      gesture_id: gestureId,
      glyph_id: gesture.glyph_id || gestureId,
  ...
  if (!steps.length) push('current-state → target-held', ['somatic.observe-hold'], { target_id: target.target_id }, 'target-held');
...
export function createSomaticCartographyReceipt({ course, step, status, observed_state_id = null, pending_id = null, capability_receipt_ids = [] }, { now = () => new Date() } = {}) {
  if (course?.schema !== SOMATIC_COURSE_SCHEMA) throw new Error('Somatic cartography receipt requires a course.');
  const courseStep = course.steps?.find((item) => item.step === step);
  if (!courseStep) throw new Error('Somatic cartography receipt step is not present in course.');
  if (!['applied', 'observed', 'pending', 'skipped', 'failed'].includes(status)) throw new Error('Somatic cartography receipt status is unsupported.');
  const recordedAt = nowIso(now);
  const capabilityIds = cleanList(capability_receipt_ids);
  return deepFreeze({
    schema: SOMATIC_CARTOGRAPHY_RECEIPT_SCHEMA,
    receipt_id: stableId('somatic-cartography-receipt', [course.course_id, step, status, observed_state_id, pending_id, recordedAt, JSON.stringify(capabilityIds)]),
    course_id: course.course_id,
    step,
    transition: courseStep.transition,
    status,
    observed_state_id: clean(observed_state_id),
    pending_id: clean(pending_id),
...
export function createSomaticStore({ initialState = null } = {}) {
  ...
  return Object.freeze({
    ...
    appendReceipt: (receipt) => {
      if (receipt?.schema !== SOMATIC_CARTOGRAPHY_RECEIPT_SCHEMA) throw new Error('Somatic store accepts somatic-cartography-receipt/v1 only.');
