...
function freezeRecord(value) { return Object.freeze(clone(value)); }
...
export function createEventBus({ definitions = OS_EVENT_DEFINITIONS, historyLimit = DEFAULT_HISTORY_LIMIT, now } = {}) {
  ...
  function requireKnownEvent(name) { if (!validators.has(name)) throw new Error(`Unknown ArcSweep OS event: ${name}`); }
  ...
  function unsubscribe(name, id) { return listeners.get(name)?.delete(id) || false; }
  function publish(name, payload = {}, meta = {}) {
    ...
    const receipt = freezeRecord({
      ...
      event_id: createId('event'), sequence, name,
      payload: clone(payload), meta: clone(meta), emitted_at: nowIso(now),
    });
  ...
  return Object.freeze({ define, subscribe, unsubscribe, publish,
...
export function createContextCapsule({ session, previousRoom = null, currentRoom, patch = {}, relevantObjects = [], openWork = [], authorityBoundary = null, receiptIds = [] } = {}, { now } = {}) {
  ...
  return freezeRecord({
    ...
    capsule_id: createId('context'), session_id: session.session_id,
    ...
    previous_room: previousRoom, current_room: currentRoom,
    current_goal: patch.current_goal ?? session.current_goal ?? null,
    relevant_objects: clone(relevantObjects), open_work: clone(openWork),
    authority_boundary: authorityBoundary, receipt_ids: clone(receiptIds), created_at: nowIso(now),
  });
}
...
export function createCheckpointStore({ now } = {}) {
  ...
  return Object.freeze({
    capture(subject, { label = null, metadata = {} } = {}) {
      const checkpoint = freezeRecord({ schema: ARCSWEEP_OS_MANIFEST.contracts.checkpoint,
        checkpoint_id: createId('checkpoint'), label, state: clone(subject), metadata: clone(metadata), created_at: nowIso(now) });
      checkpoints.set(checkpoint.checkpoint_id, checkpoint);
      return clone(checkpoint);
    },
    read(id) { const item = checkpoints.get(id); return item ? clone(item) : null; },
    restore(id) { const item = checkpoints.get(id); if (!item) throw new Error(`Unknown checkpoint: ${id}`); return clone(item.state); },
    remove(id) { return checkpoints.delete(id); },
  });
}

export function createServiceHealth(input = {}, { now } = {}) {
  const allowed = new Set(['healthy', 'degraded', 'failed', 'paused']);
  return freezeRecord({ schema: ARCSWEEP_OS_MANIFEST.contracts.health,
    service_id: input.service_id,
    status: allowed.has(input.status) ? input.status : 'degraded',
...
export function createRepairReceipt(input = {}, { now } = {}) {
  ...
  return freezeRecord({ schema: ARCSWEEP_OS_MANIFEST.contracts.repairReceipt,
    repair_id: input.repair_id || createId('repair'),
    detected_at: input.detected_at || nowIso(now),
    fault_class: input.fault_class || 'UNKNOWN', service_id: input.service_id || 'unknown',
    before_checkpoint: input.before_checkpoint || null, repair_level: input.repair_level || 'R0',
    action: input.action || 'observe', authority_sources: clone(input.authority_sources || []),
    validation: clone(input.validation || []), result,
