...
function receiptId(receipt) {
  return receipt?.call_id || receipt?.event_id || null;
}
...
export function registerSomaticService(registry, {
  ...
  if (!registry?.registerService || !registry?.registerCapability || !registry?.invoke) {
    throw new Error('Somatic service requires the ArcSweep capability registry.');
  }
  ...
  registry.registerCapability({
    ...
    service_id: 'somatic-cartography',
    description: 'Read the active somatic state, receipts, supported world profiles, and pending embodied transitions.',
    authority: 'read',
    execute: () => ({
