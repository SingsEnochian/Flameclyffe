...
export function installArcSweepOSShell({ os } = {}) {
  ...
  host.dataset.arcsweepOsShell = 'v0.2';
  ...
  host.innerHTML = `
    <style>
      ...
      [data-os-panel]{width:min(430px,calc(100vw - 28px));margin-top:8px;border:1px solid rgba(220,180,95,.45);border-radius:16px;background:rgba(18,17,22,.97);box-shadow:0 18px 54px rgba(0,0,0,.36);overflow:hidden}
  ...
  host.querySelector('[data-os-guide]').addEventListener('submit', (event) => {
    ...
    const listener = (responseEvent) => {
      ...
      reply.textContent = text(turn.say || turn.status || 'No Guide response.');
