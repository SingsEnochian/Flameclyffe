...
export function createCaretaker({ bus, checkpointStore, healthRegistry, repairBudget = createRepairBudget(), now } = {}) {
  ...
  function registerRequiredDerivedState({ stateId, serviceId = 'arcsweep-os', probe, captureState, repair = null, rollback = null, verifyRollback = null, repairLevel = 'R1' } = {}) {
    ...
    requiredDerivedStates.set(stateId, { stateId, serviceId, probe, captureState, repair, rollback, verifyRollback, repairLevel });
    return stateId;
  }
  ...
  async function repairService(fault, descriptor) {
    return runRepairTransaction({
      ...
      captureState: descriptor.captureState || (async () => ({ health: healthRegistry?.get?.(descriptor.serviceId) || null, probe: await probeDescriptor(descriptor) })),
