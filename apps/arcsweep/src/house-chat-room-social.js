...
const PORTRAIT_GLYPHS = Object.freeze({
  lioreal: 'VL', uial: 'FU', larkshine: '✦', ellowind: '◌', altair: 'A', atlas: 'AT',
  runeweaver: 'ᚱ', boxfire: '▣', yggdrasil: 'Y', bluebird: 'B', vethrlauf: 'V', rowan: 'R',
});
...
export function voiceIdentityForLabel(label = '', voices = CONSTELLATION_VOICES) {
  ...
  if (normal === 'rowan') return { id: 'rowan', name: 'Rowan', glyph: 'R', roles: ['Steward'] };
  ...
  return { id: voice.id, name: voice.name, glyph: PORTRAIT_GLYPHS[voice.id] || initialsForName(voice.name), roles: Array.isArray(voice.roles) ? voice.roles : [] };
}
...
export function activePresenceSentence(records = []) {
  ...
  const verb = active.some((record) => record.state === 'speaking') ? 'answering' : 'thinking';
...
export function presenceCounts(records = []) {
  return (records || []).reduce((counts, record) => {
    counts.total += 1;
    if (['ready', 'thinking', 'speaking'].includes(record?.state)) counts.available += 1;
    if (['thinking', 'speaking'].includes(record?.state)) counts.active += 1;
    return counts;
...
function setExpanded(node, value) { if (node && node.getAttribute('aria-expanded') !== String(value)) node.setAttribute('aria-expanded', String(value)); }

function participantState() {
  const selected = new Set([...document.querySelectorAll('#commons-form input[name="voiceIds"]:checked')].map((input) => input.value));
  const presence = new Map(currentModelPresence().map((record) => [record.voice_id, record]));
  return { selected, presence };
}

function participantMarkup(selected, presence) {
  const voices = CONSTELLATION_VOICES.map((voice) => {
    const identity = voiceIdentityForLabel(voice.id);
    const state = presence.get(voice.id)?.state || 'offline';
    return `<button type="button" class="house-room-participant" data-room-participant="${esc(voice.id)}" data-state="${esc(state)}" aria-pressed="${selected.has(voice.id)}" title="Route next unmentioned turn to ${esc(voice.name)}"><span class="house-room-avatar" aria-hidden="true">${esc(identity.glyph)}</span><span class="house-room-nameplate"><strong>${esc(voice.name)}</strong><small>${esc(identity.roles[0] || 'Constellation')} · ${esc(state)}</small></span><span class="house-room-presence-dot" aria-hidden="true"></span></button>`;
  }).join('');
  return `<div class="house-room-participant house-room-participant-self" data-state="ready"><span class="house-room-avatar" aria-hidden="true">R</span><span class="house-room-nameplate"><strong>Rowan</strong><small>Steward · you</small></span><span class="house-room-presence-dot" aria-hidden="true"></span></div>${voices}`;
}

function chromeMarkup() {
  return `<header class="house-room-header"><div class="house-room-heading"><span class="house-room-mark" aria-hidden="true">∞</span><div><strong data-house-room-title>Constellation Room</strong><small data-house-room-subtitle>House Chat · persistent room</small></div></div><div class="house-room-actions"><span class="house-room-online" data-house-room-online></span><button type="button" class="quiet mini" data-house-room-home>Home</button><button type="button" class="quiet mini" data-house-room-rooms aria-expanded="false">Rooms</button><button type="button" class="quiet mini" data-house-room-routing aria-expanded="false">Routing</button></div></header><nav class="house-room-participants" data-house-room-participants aria-label="House Chat participants"></nav>`;
}
...
function ensureChrome(form) {
  ...
  if (!chrome) {
    ...
    chrome.dataset.houseRoomChrome = 'true'; chrome.className = 'house-room-chrome'; chrome.innerHTML = chromeMarkup(); layout.prepend(chrome);
  }
  return chrome;
}
...
function renderParticipants(chrome, form) {
  ...
  if (!nav) return;
  const { selected, presence } = participantState();
  const fingerprint = JSON.stringify(CONSTELLATION_VOICES.map((voice) => [voice.id, selected.has(voice.id), presence.get(voice.id)?.state || 'offline']));
  if (nav.dataset.fingerprint === fingerprint) return;
  nav.dataset.fingerprint = fingerprint;
  const scroll = nav.scrollLeft;
  nav.innerHTML = participantMarkup(selected, presence);
...
function updateRoomHeading(chrome) {
  ...
  setText(chrome.querySelector('[data-house-room-title]'), title);
  setText(chrome.querySelector('[data-house-room-subtitle]'), current === home ? 'House Chat · persistent home room' : current ? 'House Chat · conversation' : 'House Chat · first message opens a new room');
}
...
function restoreOrClaimHome(chrome) {
  ...
  if (!saved && current) { writeHomeThread(current); ensureHomeTitle(current); }
    ...
    select.value = saved; select.dispatchEvent(new Event('change', { bubbles: true })); return;
  }
...
function syncDrawers(form, chrome) {
  ...
  if (fieldset) { fieldset.dataset.houseRoomRouting = 'true'; if (fieldset.hidden === routingOpen) fieldset.hidden = !routingOpen; }
  const drawer = document.querySelector('[data-commons-command-room]');
  if (drawer) { drawer.classList.add('house-room-drawer'); if (drawer.hidden === drawerOpen) drawer.hidden = !drawerOpen; }
  setExpanded(chrome.querySelector('[data-house-room-rooms]'), drawerOpen);
  setExpanded(chrome.querySelector('[data-house-room-routing]'), routingOpen);
}
...
function decorateEntry(article) {
  ...
  article.dataset.houseRoomDecorated = 'true'; article.dataset.houseRoomAuthor = identity.id;
...
function revealNewVoiceEntry(article) {
  ...
  body.textContent = ''; article.classList.add('house-room-revealing');
  let index = 0;
  ...
  const paint = () => {
    ...
    const log = body.closest('.commons-log'); if (log) log.scrollTop = log.scrollHeight;
...
function renderLivePresence(chrome) {
  ...
  setText(chrome.querySelector('[data-house-room-online]'), counts.active ? `${counts.active} active · ${counts.available} available` : `${counts.available} available`);
  ...
  if (!sentence) { if (typing) typing.remove(); return; }
  const identity = voiceIdentityForLabel(active[0]?.voice_id || '') || { glyph: '∞' };
  const fingerprint = JSON.stringify([sentence, identity.glyph]);
  if (typing?.dataset.fingerprint === fingerprint) return;
  if (!typing) { typing = document.createElement('div'); typing.dataset.houseRoomTyping = 'true'; typing.className = 'house-room-typing'; log.append(typing); }
  typing.dataset.fingerprint = fingerprint;
  typing.innerHTML = `<span class="house-room-avatar" aria-hidden="true">${esc(identity.glyph)}</span><span><strong>${esc(sentence)}</strong><span class="house-room-typing-dots" aria-hidden="true"><i></i><i></i><i></i></span></span>`;
...
function wireComposer(form) {
  ...
  if (hint) setText(hint, 'Enter sends · Shift+Enter adds a line · native rich text · @name routes directly.');
}
