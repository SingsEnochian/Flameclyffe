...
export const FIRST_LIVING_PAGE_SURFACE = 'arcsweep.first-living-page-surface/v0.1';
...
function threadMarkup() {
  ...
  if (!messages.length) {
    return '<p class="living-empty">No words yet. This is a real beginning, not reconstructed history.</p>';
  }
...
function livingMarkup() {
  ...
  return `
    <style>
      ...
      .living-lantern{width:68px;height:82px;border:1px solid rgba(91,61,37,.4);border-radius:22px 22px 28px 28px;background:radial-gradient(circle at 50% 55%,rgba(255,235,157,.95),rgba(211,132,42,.72) 36%,rgba(71,43,26,.9) 70%);box-shadow:inset 0 0 20px rgba(255,255,255,.26);cursor:pointer;transform:translateZ(0);transition:filter .3s ease,transform .3s ease,box-shadow .3s ease}
      ...
      .living-empty{opacity:.62;font-style:italic}
      .living-seam{padding:8px 10px;border-left:3px solid rgba(91,61,37,.28);font-size:12px;opacity:.78}
    ...
    <section data-first-living-page-root data-revision="${revision}" data-lantern-state="${esc(state.lantern.state)}">
      <div>
        <p class="living-kicker">First Living Page · inhabited continuity</p>
        <h2>The Lantern Room</h2>
        <p>This page keeps the arrows: encounter → change → return → next encounter.</p>
      </div>
      ...
      <div class="living-grid">
        <section class="living-card">
          <h3>Speak into the page</h3>
          <div class="living-thread" role="log" aria-live="polite">${threadMarkup()}</div>
          <form class="living-form" data-living-form>
            <textarea name="utterance" aria-label="Speak to the inhabitant" placeholder="Write into the Lantern Room…" ${busy ? 'disabled' : ''}></textarea>
            <button type="submit" ${busy ? 'disabled' : ''}>${busy ? 'Listening through the receiver…' : 'Speak'}</button>
          </form>
          <p class="living-status" aria-live="polite">${esc(statusMessage)}</p>
        </section>
        <section class="living-card">
          <h3>The wake</h3>
          <div class="living-lineage">${lineageMarkup()}</div>
        </section>
      </div>
      <section class="living-card">
        <h3>Operative objects</h3>
        <p><strong>Lantern:</strong> ${esc(state.lantern.state)} after ${state.lantern.touch_count} touch${state.lantern.touch_count === 1 ? '' : 'es'}.</p>
        <p><strong>Glyph imprint:</strong> ${esc(glyphMarkup())}</p>
        <div class="living-actions">
          <button type="button" data-living-glyph-imprint>Imprint current glyph</button>
          <button type="button" data-living-show-threshold>Return to Threshold</button>
        </div>
      </section>
...
function ensureNavButton() {
  ...
  const nav = root?.querySelector('.magic-book-binding-nav');
  if (!nav) return null;
...
function renderLivingPage() {
  ...
  const mounted = target.querySelector('[data-first-living-page-root]');
  if (mounted) mounted.dataset.busy = String(busy);
...
function leaveLivingPage(reason = 'native-page-selected') {
  ...
  livingMode = false;
  const root = document.getElementById(ROOT_ID);
  root?.querySelector(`[${NAV_BUTTON_ATTR}]`)?.removeAttribute('aria-current');
}
...
function guidePrompt(message) {
  return [
    ...
    'Use the supplied page lineage and recent thread as causal ancestry. Do not invent missing history.',
    'The continuity address and the current model receiver are not automatically identical; preserve that seam in provenance.',
...
function requestGuide(message) {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      ...
      reject(new Error('The active ArcSweep receiver did not answer before the page timed out.'));
    }, RESPONSE_TIMEOUT_MS);
    const listener = (event) => {
      if (event?.detail?.request_id !== requestId) return;
      clearTimeout(timer);
      globalThis.removeEventListener?.('arcsweep:guide-response', listener);
      resolve(event.detail.turn || {});
    };
...
async function speak(message) {
  ...
  try {
    ...
    statusMessage = turn.status === 'replied'
      ? 'Answer received. The encounter is now part of the page lineage.'
      : `Receiver status: ${text(turn.status, 120) || 'unknown'}.`;
  } catch (error) {
    commit(recordLivingPageTurn(state, {
      role: 'system',
      content: error?.message || String(error),
      actorId: 'arcsweep:first-living-page',
    }));
...
function touchLantern() {
  ...
  statusMessage = state.lantern.state === 'lit'
    ? 'Lantern lit. The room has changed and the change has ancestry.'
    : 'Lantern banked. The prior lit state remains in the wake.';
  globalThis.dispatchEvent?.(new CustomEvent('arcsweep:first-living-page-lantern', {
    detail: { state: state.lantern.state, touch_count: state.lantern.touch_count },
  }));
  renderLivingPage();
}
...
function handleRootClick(event) {
  if (!livingMode) return;
  if (event.target.closest?.('[data-living-lantern]')) {
    event.preventDefault();
    touchLantern();
    return;
  }
  if (event.target.closest?.('[data-living-glyph-imprint]')) {
    event.preventDefault();
    imprintGlyph();
    return;
  }
...
function handleNativeNavigation(event) {
  if (!livingMode) return;
  if (event.target.closest?.(`[${NAV_BUTTON_ATTR}]`)) return;
  const native = event.target.closest?.('[data-magic-book-page],[data-magic-book-page-prev],[data-magic-book-page-next],[data-generator-atelier-open]');
  if (native) leaveLivingPage('native-page-selected');
}
