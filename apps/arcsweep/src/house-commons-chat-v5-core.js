...
export function roomContext(entries = [], roomId = '', limit = 24) {
  if (!roomId) return [];
  return entries.filter((entry) => commonsThreadId(entry) === roomId && entry?.text && !String(entry.id || '').startsWith('optimistic:')).slice(-limit).map((entry) => ({
    speaker: String(entry.author || 'House').slice(0, 120),
    text: String(entry.text || '').slice(0, 6000),
  }));
}

export function roomEntries(entries = [], roomId = '', { search = '', pinnedIds = null } = {}) {
  const needle = String(search || '').trim().toLowerCase();
  const pins = pinnedIds ? new Set(pinnedIds) : null;
  return entries.filter((entry) => {
    if (roomId && commonsThreadId(entry) !== roomId) return false;
    if (pins && !pins.has(entry.id)) return false;
    if (!needle) return true;
    return [entry.author, entry.text, entry.voice_id, entry.runtime?.model, entry.runtime?.provider].filter(Boolean).join(' ').toLowerCase().includes(needle);
  });
}

export function latestRoomEntry(entries = [], roomId = '') {
...
export function deliveryState(entry) {
  if (entry?.optimistic && entry?.status === 'failed') return 'failed';
  if (entry?.optimistic) return 'sending';
  if (entry?.kind === 'steward') return entry?.status === 'sent' ? 'sent' : entry?.status || 'delivered';
  return entry?.status || 'received';
}

export function roomLabel(room, unread = 0) {
  const prefix = room?.kind === 'direct' ? '@' : '#';
  return `${prefix}${room?.slug || room?.title || 'room'}${unread ? ` (${unread})` : ''}`;
}

export function directRoomId(voiceId) {
...
export function directRoomSeed(voiceId, voices = CONSTELLATION_VOICES) {
  ...
  return {
    id: directRoomId(voice.id),
    slug: voice.id,
    title: voice.name,
    topic: `Direct House room with ${voice.name}.`,
    kind: 'direct',
