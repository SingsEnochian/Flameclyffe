...
export async function buildSceneCortexAcceptanceReport(fieldContext, options = {}) {
  ...
  const subjectCells = (packet.subjects || []).flatMap((subject) => subject.cells || []);
  const activeOldName = cellContains(subjectCells, 'Kestrelle al’Valar') || cellContains(subjectCells, "Kestrelle al'Valar");
  const activeOldEra = cellContains(subjectCells, 'Mending');
  const checks = [
    ...
    check('active-name', 'Active protagonist terminology present', Boolean(character && cellContains(character.cells, 'Kestrelle al’Var')), 'Kestrelle al’Var expected in active character context'),
    check('superseded-name', 'Superseded protagonist name excluded from active subject context', !activeOldName, activeOldName ? 'superseded al’Valar term leaked into active cells' : 'historical name remains outside ordinary activation'),
    check('superseded-era', 'Superseded era term excluded from active subject context', !activeOldEra, activeOldEra ? 'Mending leaked into active subject cells' : 'Restoration context remains active'),
  ];

  const required = checks.filter((item) => item.severity === 'required');
  return {
    contract: 'arcsweep.scene-cortex-acceptance-report/v1',
...
export async function inspectSceneCortexControl(control, options = {}) {
  ...
  const context = buildFieldContext(control, 'scene-cortex-acceptance');
  return buildSceneCortexAcceptanceReport(context, options);
}
