...
export const BLUEBIRD_WEIGHTED_HOME = Object.freeze({
  ...
  coupledHearts: Object.freeze({
    bpm: 55,
    cycleSeconds: 60 / 55,
    answerOffsetCycles: .5,
    lubDubGapSeconds: .18,
    label: 'Two 55 BPM lub-dub hearts in a closed call-and-response loop',
    voices: Object.freeze([
      Object.freeze({ id: 'bluebird-heart', label: 'Bluebird heart', pan: -.32, lubHz: 58, dubHz: 46 }),
      Object.freeze({ id: 'rowan-heart', label: 'Rowan heart', pan: .32, lubHz: 54, dubHz: 43 }),
    ]),
  }),
...
export function validateBluebirdWeightedHome(profile = BLUEBIRD_WEIGHTED_HOME) {
  ...
  if (Math.abs((60 / profile.coupledHearts.cycleSeconds) - profile.coupledHearts.bpm) > 1e-9) errors.push('coupled-heart BPM mismatch');
  if (profile.coupledHearts.voices.length !== 2) errors.push('coupled-heart loop requires two distinct voices');
...
export function createBluebirdWeightedHomeReceipt({ mode = 'stereo', soundfontVoiceIds = [], somaticProxy = false, startedAt = new Date().toISOString() } = {}) {
  ...
  return Object.freeze({
    ...
    render: Object.freeze({ mode, binaural_beat_hz: mode === 'stereo' ? 5.5 : null, amplitude_modulation_hz: mode === 'mono' ? 5.5 : null, coupled_hearts_bpm: BLUEBIRD_WEIGHTED_HOME.coupledHearts.bpm, coupled_hearts_pattern: 'lub-dub / answering lub-dub', soundfont: soundfontVoiceIds.length > 0, soundfont_voice_ids: Object.freeze([...soundfontVoiceIds]), somatic_audio_proxy: Boolean(somaticProxy) }),
