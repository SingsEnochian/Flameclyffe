...
async function render(root) {
  ...
  root.innerHTML = `
    ...
    <div class="constellation-presence-panel" hidden>
      ...
      <p class="constellation-presence-note">Selection grants context participation only. It does not grant tool writes, canon commits, or silent edits.</p>
    </div>
  `;
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .constellation-presence-panel { margin-top:.45rem; max-height:min(70vh,34rem); overflow:auto; padding:.85rem; border:1px solid color-mix(in srgb,var(--gold) 30%,transparent); border-radius:1rem; background:color-mix(in srgb,var(--panel-solid) 96%,black); box-shadow:0 .7rem 2.2rem rgb(0 0 0 / .38); }
