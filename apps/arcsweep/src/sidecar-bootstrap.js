...
const SIDECARS = Object.freeze([
  ...GLOBAL_SIDECARS,
  ...new Set(Object.values(SIDECAR_PACKS).flat()),
]);

// Keep every runnable sidecar statically visible to Vite without executing it at boot.
// Runtime scheduling below decides *when* each chunk is imported.
...
function diagnostics() {
  if (!globalThis.__arcsweepSidecarDiagnostics) {
...
async function loadSidecar(specifier, pack = 'global') {
  ...
  const promise = (async () => {
    ...
    globalThis.dispatchEvent?.(new CustomEvent('arcsweep:sidecar-loaded', {
      detail: { specifier, pack, elapsedMs },
    }));
...
export async function mountSidecarPack(name) {
  ...
  const promise = (async () => {
    const results = [];
    for (const specifier of SIDECAR_PACKS[name]) {
      results.push(await loadSidecar(specifier, name));
      await yieldToBrowser();
    }
    diagnostics().packs.push({ name, completedAt: new Date().toISOString() });
    globalThis.dispatchEvent?.(new CustomEvent('arcsweep:sidecar-pack-ready', {
      detail: { name, failures: results.filter((item) => item?.message) },
    }));
    return results;
  })();
...
const ROOM_PACKS = Object.freeze({
  worlds: ['worlds'],
  scripts: ['writing'],
  records: ['writing'],
  feedback: ['feedback'],
...
function packNamesForTrigger(node) {
  ...
  const values = [
    node.dataset?.room,
    node.dataset?.roomId,
    node.dataset?.collectionRoom,
    node.dataset?.recordRoom,
  ].filter(Boolean);
  const names = new Set();
...
function triggerDetectedPacks() {
  const selectors = [
    ['house', '#commons-form'],
    ['worlds', '#world-registry-form'],
    ['feedback', '#feedback-form'],
    ['aemeth', '#record-form[data-room-id="aemeth-lens"]'],
    ['worldseed', '#record-form[data-room-id="seedhouse"]'],
    ['canon', '#record-form[data-room-id="ingest"]'],
  ];
  for (const [name, selector] of selectors) {
    if (document.querySelector(selector)) void mountSidecarPack(name);
  }
}
...
export async function mountArcsweepSidecars() {
  const results = [];
  for (const specifier of GLOBAL_SIDECARS) {
    results.push(await loadSidecar(specifier, 'global'));
    await yieldToBrowser();
  }
  installPackScheduler();

  const params = new URLSearchParams(globalThis.location?.search || '');
  if (params.get('sidecars') === 'full') {
    for (const name of Object.keys(SIDECAR_PACKS)) await mountSidecarPack(name);
  } else {
    triggerDetectedPacks();
  }

  const failures = results.filter((item) => item?.message);
  globalThis.dispatchEvent?.(new CustomEvent('arcsweep:sidecars-ready', {
    detail: { failures, scope: 'global', lazyPacks: true },
  }));
  return failures;
}
