...
function enhance() {
  ...
  document.querySelectorAll('.commons-chat-entry').forEach((entry) => {
    if (!entry.querySelector('[data-attach-to-entry]')) { const button = document.createElement('button'); button.type = 'button'; button.className = 'quiet mini'; button.dataset.attachToEntry = 'true'; button.textContent = 'Attach'; entry.querySelector('header>div')?.prepend(button); button.addEventListener('click', () => { const picker = document.createElement('input'); picker.type = 'file'; picker.multiple = true; picker.onchange = async () => { await attachFiles([...picker.files], entry.dataset.entryId, entry.dataset.threadId); document.dispatchEvent(new CustomEvent('arcsweep:commons-attachment-saved')); }; picker.click(); }); }
  });
  document.querySelectorAll('.commons-chat-entry').forEach((entry) => { const body = entry.querySelector('.commons-chat-body'); const text = body?.textContent || ''; if (!/Attached \d+ files|Attached .+\./.test(text)) return; });
}
function renderAttachmentChips() {
  fetch('/api/v1/house/commons', { credentials: 'same-origin', cache: 'no-store' }).then((response) => response.ok ? response.json() : null).then((data) => {
    for (const item of data?.entries || []) { if (!item.attachments?.length) continue; const node = document.querySelector(`[data-entry-id="${CSS.escape(item.id)}"]`); if (!node || node.querySelector('[data-attachment-chip]')) continue; const wrap = document.createElement('div'); wrap.className = 'commons-links'; for (const attachment of item.attachments) { const button = document.createElement('button'); button.type = 'button'; button.className = 'commons-link-chip'; button.dataset.attachmentChip = attachment.id; button.textContent = `${attachment.name}${attachment.size != null ? ` · ${Math.ceil(attachment.size / 1024)} KiB` : ''}`; button.addEventListener('click', () => void openAttachment(attachment.id)); wrap.append(button); } node.append(wrap); }
  }).catch(() => null);
}
