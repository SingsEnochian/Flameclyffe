...
function renderStorySoundscape(sound = storySoundscape.snapshot()) {
  ...
  return `<section class="story-soundscape" data-story-soundscape>
    ...
    <div class="soundfont-rack bluebird-weighted-home" data-bluebird-weighted-home>
      <div><p class="eyebrow">Runa coordination preset · bounded audition</p><h4>${escapeHtml(bluebird.profile.name)}</h4><p>${escapeHtml(bluebird.profile.purpose)}</p><p class="muted">432 Hz left / 437.5 Hz right · Δ 5.5 Hz · ${bluebird.profile.somaticProxy.carrierHz} Hz tactile-audio proxy pulsed at ${bluebird.profile.somaticProxy.pulseBpm} BPM · eight-minute automatic return.</p></div>
