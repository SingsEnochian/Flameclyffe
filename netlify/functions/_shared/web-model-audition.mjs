...
function isWebDirect(candidate) {
  return candidate?.runtime?.provider === 'openai-compatible'
    && Boolean(candidate.runtime?.base_url || candidate.runtime?.base_url_env);
}

function candidateBaseUrl(candidate, env) {
  return String(env.get(candidate.runtime?.base_url_env) || candidate.runtime?.base_url || '').trim().replace(/\/$/, '');
}
...
async function callWebDirect(manifest, candidate, body, env, fetchImpl) {
  ...
  const base = candidateBaseUrl(candidate, env);
  if (!base) throw new Error(`Missing server configuration: ${candidate.runtime.base_url_env}. This model needs a running inference endpoint; a Hub repository or storage bucket is not an inference service.`);
  ...
  const sampling = candidate.runtime?.sampling || {};
  if (Number.isFinite(sampling.temperature)) payload.temperature = sampling.temperature;
  if (Number.isFinite(sampling.top_p)) payload.top_p = sampling.top_p;
  // top_k is calibration metadata until the selected backend explicitly supports it.

  const endpoint = `${base}/chat/completions`;
  const providerLabel = candidate.runtime.backend || 'Candidate inference';
  const options = {
    method: 'POST',
  ...
  try {
    data = await providerJson(fetchImpl, endpoint, options, providerLabel);
  } catch (error) {
    if (!effort || error.status !== 400) throw error;
    const retryPayload = { ...payload };
    delete retryPayload.reasoning_effort;
    data = await providerJson(fetchImpl, endpoint, { ...options, body: JSON.stringify(retryPayload) }, providerLabel);
    appliedEffort = null;
  }
  ...
  if (!visible && effort) {
    ...
    try {
      data = await providerJson(fetchImpl, endpoint, { ...options, body: JSON.stringify(visiblePayload) }, providerLabel);
      appliedEffort = 'none';
...
function statusFor(manifest, candidate, env) {
  if (isWebDirect(candidate)) {
    const credential = candidateCredential(candidate, env);
    const missing = [];
    if (!credential.value) missing.push(candidate.runtime.api_key_env);
    if (!candidateBaseUrl(candidate, env)) missing.push(candidate.runtime.base_url_env);
    return {
      flame_id: manifest.flame_id,
      display_name: manifest.display_name,
      candidate_id: candidate.candidate_id,
      ...
      backend_configured: missing.length === 0,
...
export function createWebModelAuditionHandler({ env, fetchImpl = fetch }) {
  ...
  try {
    const result = isWebDirect(candidate)
      ? await callWebDirect(manifest, candidate, body, env, fetchImpl)
      : await callLegacyGateway(manifest, candidate, body, env, fetchImpl);
