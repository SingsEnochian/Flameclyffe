import { invokeHostedFlameFallback } from './_shared/hosted-flame-fallback.mjs';
...
export default async (request) => {
  ...
  const context = String(env.get('CONTEXT') || '').trim();
  if (context && context !== 'deploy-preview' && context !== 'branch-deploy') {
    return json(404, { error: 'House Chat live smoke is preview-only.' });
  }
  ...
  try {
    ...
    return json(200, {
      ...
      messages: [
        {
          flame_id: ox.flame_id,
          display_name: ox.display_name,
          provider: ox.provider,
          model: ox.model,
          latency_ms: oxLatency,
          message: ox.message,
        },
        {
          flame_id: second.flame_id,
          display_name: second.display_name,
          provider: second.provider,
          model: second.model,
          latency_ms: secondLatency,
...
export const config = {
  path: '/api/house-chat-live-smoke',
};
